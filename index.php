...
//    echo $email." - ".$password;
    $select = $pdo->prepare("select * from user where useremail='$email' AND password='$password'");
    ...
    if($row['useremail']==$email AND $row['password']==$password AND $row['role']=='Admin') {
    ...
    }else if($row['useremail']==$email AND $row['password']==$password AND $row['role']=='User') {
        $_SESSION['userid'] = $row['userid'];
        $_SESSION['username'] = $row['username'];
        $_SESSION['useremail'] = $row['useremail'];
        $_SESSION['password'] = $row['password'];
        ...
        jQuery(function validation() {
            ...
            text: "You have failed to log In!",
            icon: "error",
            });
        });
...
<head>
  ...
  <title>POSaleSYSTEM | Log in</title>
